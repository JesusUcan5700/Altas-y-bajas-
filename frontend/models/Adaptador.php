...
class Adaptador extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['MARCA', 'MODELO', 'TIPO', 'ESTADO', 'EMISION_INVENTARIO'], 'required'],
    ...
    public function validarNumSerie($attribute, $params)
    {
        ...
        if (!$this->isNewRecord) {
            $query->andWhere(['!=', 'idAdaptador', $this->idAdaptador]);
        }

        if ($query->exists()) {
            $this->addError($attribute, 'Este número de serie ya está registrado en otro adaptador (incluso si está dado de baja).');
        }
    }
    ...
    public function validarNumInventario($attribute, $params)
    {
        ...
        if ($query->exists()) {
            $this->addError($attribute, 'Este número de inventario ya está registrado en otro adaptador (incluso si está dado de baja).');
        }
    }
