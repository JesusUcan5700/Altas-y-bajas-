...
class VideoVigilancia extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['MARCA', 'MODELO', 'NUMERO_SERIE', 'NUMERO_INVENTARIO', 'EDIFICIO', 'EMISION_INVENTARIO', 'TIEMPO_TRANSCURRIDO', 'VIDEO_VIGILANCIA_COL'], 'string', 'max' => 45],
            [['DESCRIPCION'], 'string'],
    ...
    public function validarNumSerie($attribute, $params)
    {
        ...
        if (!$this->isNewRecord) {
            $query->andWhere(['!=', 'idVIDEO_VIGILANCIA', $this->idVIDEO_VIGILANCIA]);
        }

        if ($query->exists()) {
            $this->addError($attribute, 'Este número de serie ya está registrado en otra cámara (incluso si está dado de baja).');
        }
    }
    ...
    public function validarNumInventario($attribute, $params)
    {
        ...
        if ($query->exists()) {
            $this->addError($attribute, 'Este número de inventario ya está registrado en otra cámara (incluso si está dado de baja).');
        }
    }
