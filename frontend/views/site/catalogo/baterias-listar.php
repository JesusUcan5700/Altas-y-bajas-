...
$this->title = 'Catálogo de Baterías';
            ...
            <div class="card shadow-lg border-0">
                <div class="card-header bg-warning text-dark py-3">
                    <div class="d-flex align-items-center">
                        <i class="fas fa-battery-three-quarters fa-2x me-3"></i>
                        <div>
                            <h1 class="h3 mb-0 fw-bold">Gestión de Catálogos - Baterías</h1>
                            <small class="opacity-75">Solo baterías creadas desde el formulario rápido (catálogo)</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-info alert-dismissible fade show" role="alert">
                        ...
                        <p class="mb-0">
                            <i class="fas fa-infinity me-1"></i> <strong>Reutilización infinita:</strong> Puedes usar estas baterías cuantas veces necesites sin que se agoten.<br>
                    ...
                    <?php else: ?>
                        ...
                        <?php if (empty($baterias)): ?>
                            <div class="alert alert-warning text-center" role="alert">
                                <h4><i class="fas fa-exclamation-triangle me-2"></i>No hay baterías en el catálogo</h4>
                                <p class="mb-3">Aún no has agregado baterías usando el formulario rápido.</p>
                                <a href="<?= Url::to(['site/baterias', 'simple' => 1]) ?>" class="btn btn-primary">
                                    <i class="fas fa-plus me-2"></i>Agregar Primera Batería al Catálogo
                                </a>
                            </div>
                        <?php else: ?>
                                        ...
                                        <div class="card h-100 shadow-sm border-warning">
                                            <div class="card-body">
                                                ...
                                                <div class="form-check position-absolute" style="top: 10px; left: 10px; z-index: 1;">
                                                    <input type="checkbox" name="bateria_ids[]" class="form-check-input item-checkbox" value="<?= $bateria->idBateria ?>">
                                                </div>
                                                
                                                <div class="d-flex align-items-start justify-content-between">
                                                    <div class="flex-grow-1 ps-4">
                                                        <h6 class="card-title mb-1 text-warning fw-bold">
                                                            <i class="fas fa-battery-three-quarters me-2"></i><?= Html::encode($bateria->MARCA) ?>
                                                        </h6>
                                                        <p class="card-text mb-2 text-dark fw-medium"><?= Html::encode($bateria->MODELO) ?></p>
                                                        ...
                                                        <div class="mb-2">
                                                            ...
                                                            <span class="badge bg-warning text-dark ms-1"><?= Html::encode($bateria->ESTADO) ?></span>
                                                        </div>

                                                        <!-- Tipo y Voltaje -->
                                                        <small class="text-muted d-block">
                                                            <i class="fas fa-bolt me-1"></i>Voltaje: <?= Html::encode($bateria->VOLTAJE) ?>
                                                        </small>

                                                        <!-- Número de inventario -->
                                                        <small class="text-muted">
                                                            <i class="fas fa-tag me-1"></i><?= Html::encode($bateria->NUMERO_INVENTARIO) ?>
                                                        </small>
                                                    </div>
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li>
                                                                <?= Html::a('<i class="fas fa-eye me-2"></i>Ver Detalles', ['bateria-ver', 'id' => $bateria->idBateria], ['class' => 'dropdown-item']) ?>
                                                            </li>
                                                            <li>
                                                                <?= Html::a('<i class="fas fa-edit me-2"></i>Editar', ['bateria-editar', 'id' => $bateria->idBateria], ['class' => 'dropdown-item']) ?>
                                                            </li>
                                                            <li><hr class="dropdown-divider"></li>
                                                            <li>
                                                                <a href="javascript:void(0)" onclick="eliminarItem(<?= $bateria->idBateria ?>, '<?= Html::encode($bateria->MARCA . ' ' . $bateria->MODELO) ?>')" class="dropdown-item text-danger">
                                            ...
                                            <div class="card-footer bg-light text-muted">
                                                <small>
                                                    ...
                                                    Agregado: <?= Yii::$app->formatter->asDatetime($bateria->fecha_creacion ?? 'now', 'short') ?>
                                                </small>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-6 text-end">
                            <a href="<?= Url::to(['site/baterias', 'simple' => 1]) ?>" class="btn btn-warning">
                                <i class="fas fa-plus me-2"></i>Agregar Nueva al Catálogo
                            </a>
                            <a href="<?= Url::to(['site/baterias-listar']) ?>" class="btn btn-outline-warning">
                                <i class="fas fa-list me-2"></i>Ver Todas las Baterías
                            </a>
                        </div>
                    </div>
                </div>
...
$this->registerJs(<<<JS
    ...
    function exportarPDF() {
        ...
        doc.setFontSize(16);
        doc.text('Catálogo de Baterías', 14, 20);
        doc.setFontSize(10);
        ...
        doc.save('catalogo_baterias.pdf');
    }
